...
class Spider(object):
    ...
    def export_keywords(self, keyword, export_path='./csv/'):
        keywords = list()

        data_dump_path = './tmp/' + keyword + '.dump'
        ...
        headers = [
            ...
            ("company_cnt",       "卖家竞争度"),
            ("showwin_cnt",       "橱窗数"),
            ("srh_pv_this_mon",   self.months_ago_str(0)),
            ("srh_pv_last_1mon",  self.months_ago_str(1)),
            ("srh_pv_last_2mon",  self.months_ago_str(2)),
            ("srh_pv_last_3mon",  self.months_ago_str(3)),
            ("srh_pv_last_4mon",  self.months_ago_str(4)),
            ("srh_pv_last_5mon",  self.months_ago_str(5)),
            ("srh_pv_last_6mon",  self.months_ago_str(6)),
            ("srh_pv_last_7mon",  self.months_ago_str(7)),
            ("srh_pv_last_8mon",  self.months_ago_str(8)),
            ("srh_pv_last_9mon",  self.months_ago_str(9)),
            ("srh_pv_last_10mon", self.months_ago_str(10)),
            ("srh_pv_last_11mon", self.months_ago_str(11)),
            ("isP4pKeyword",      "P4P关键词"),
            ("yyyymm",            "更新时间")
        ]
        data = list()
        ...
        export_filename = os.path.join(export_path, keyword + '.csv')
        with open(export_filename, 'w', encoding='utf-8-sig') as export_file:
            dict_writer = csv.DictWriter(export_file, fieldnames = [x[0] for x in headers])
